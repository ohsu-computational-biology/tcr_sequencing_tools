...
gini10 <- numeric(length(clone.files.in.dir))
true10 <- numeric(length(clone.files.in.dir))
gini20 <- numeric(length(clone.files.in.dir))
true20 <- numeric(length(clone.files.in.dir))
...
print(i)
    ...
    ## Re-order so I can take top 10 and 20
    clone.curr.record <- clone.curr.record[order(clone.curr.record[[column_v]], decreasing = T),]

    ## Set new top, if max is reached
    if (nrow(clone.curr.record) < 10) {ten_v <- nrow(clone.curr.record)} else {ten_v <- 10}
    if (nrow(clone.curr.record) < 20) {twenty_v <- nrow(clone.curr.record)} else {twenty_v <- 20}
    ...
    gini10[i] <- gini(.data = clone.curr.record[[column_v]][1:ten_v], .do.norm = F)
    true10[i] <- diversity(.data = clone.curr.record[[column_v]][1:ten_v], .do.norm = F)

    gini20[i] <- gini(.data = clone.curr.record[[column_v]][1:twenty_v], .do.norm = F)
    true20[i] <- diversity(.data = clone.curr.record[[column_v]][1:twenty_v], .do.norm = F)
...
output.df <- data.frame(clone.files.in.dir, calculated.entropies, norm.entropy, unique.clones, clonality,
	     		adaptive.clonality, max.clonal.freq, max.clone.count, gini, gini10, gini20, true, true10, true20)

colnames(output.df) <- c("File", "Shannon Entropy", "Normalized Entropy", "Unique Clonotypes", "Clonality",
                         "Adaptive Clonality", "Max Clonal Freq", "Max Clone Count", "Gini_Index", "top10_Gini", "top20_Gini",
			 "True_Diversity", "top10_True", "top20_True")

### Write output
file.name <- paste0(batch_v, "_top.txt")

...
unique.clones <- NULL
clonality <- NULL
spike.counts <- numeric(length(clone.files.in.dir));
spike.percent <- numeric(length(clone.files.in.dir));
...
#    max.clone.count[i] <- max(clone.curr.record$`Normalized.clone.count`)
    ...
    ## Change frequency column to a percentage
...
#   write output
file.name <- "uniques.shannon.clonality.pct.txt"

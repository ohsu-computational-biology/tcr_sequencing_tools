...
gini <- numeric(length(clone.files.in.dir)); top10gini <- gini; top20gini <- gini
trueD <- numeric(length(clone.files.in.dir)); top10true <- trueD; top20true <- trueD
...
for(i in 1:length(clone.files.in.dir))	{
 ...
 #    column <- "Normalized.clone.fraction"
    ...
    gini[i] <- gini(.data = clone.curr.record[[column]], .do.norm = T)
    trueD[i] <- diversity(.data = clone.curr.record[[column]], .do.norm = T)

    ## Sort
    clone.curr.record <- clone.curr.record[order(clone.curr.record[[column]], decreasing = T),]
    top10.clones <- clone.curr.record[1:10,]
    top20.clones <- clone.curr.record[1:20,]

    ## Gini and True
    top10gini[i] <- gini(.data = top10.clones[[column]], .do.norm = T)
    top20gini[i] <- gini(.data = top20.clones[[column]], .do.norm = T)

    top10true[i] <- diversity(.data = top10.clones[[column]], .do.norm = T)
    top20true[i] <- diversity(.data = top20.clones[[column]], .do.norm = T)
...
output.df <- data.frame(clone.files.in.dir, calculated.entropies, norm.entropy, unique.clones, clonality,
	     		adaptive.clonality, max.clonal.freq, max.clone.count, gini, top10gini, top20gini,
                        trueD, top10true, top20true)
...
file.name <- paste0(batch_v, "tcR.txt")

...
formatted.vector <- paste("#!/bin/sh\n",
                          ...
                          "ref_dir=$ENV(tool)/nb_norm/",
                          "log_dir=$ENV(data)/condor_logs/normalization/",
                          "normalization_dir=$ENV(data)/nb_normalization/",
...
for (i in 1:length(list.of.clone.files))   {
   ...
   if (clone.index == count.index){
       ...
       formatted.vector[i+1] <- paste(
           "output=$(log_dir)stdout_nb_normalize_clonotypes_", index, ".out\n",
           "error=$(log_dir)stderr_nb_normalize_clonotypes_", index, ".out\n",
           "log=$(log_dir)condor_nb_normalize_clonotypes_", index, ".log\n",
...
out.file <- file.path(out.dir, "81_normalize.submit")

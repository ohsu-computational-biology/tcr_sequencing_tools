...
to.remove.forward <- arguments[2];  # /home/exacloud/lustre1/CompBio/data/tcrseq/dhaarini/DNAXXXXLC/
    ...
    for (i in 1:length(sorted)) {
       ...
       formatted.vector[i] <- paste(
            "output=$(log_dir)logs/stdout_remove_spikes_parallel_", i, ".out\n",
            "error=$(log_dir)logs/stderr_remove_spikes_parallel_", i, ".out\n",
            "log=$(log_dir)logs/remove_spikes_parallel_", i, ".log\n",

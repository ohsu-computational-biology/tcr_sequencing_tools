...
optlist <- list(
  ...
  make_option(
    c("-b", "--assembleReports"),
    type = "character",
    help = "Report files generated by MiXCR assemble."
  ),
  make_option(
    c("-c", "--assembleNames"),
    type = "character",
...
#                   -8 alignReports -9 alignNames -a alignOutput -b assembleReports -c assembleNames -d assembleOutput -e decontamQCFiles -f decontamQCNames
#                   -g decontamOutput -i decontamCloneFiles -j decontamCloneNames -k normCloneFiles -l normCloneNames -m normOutput -n debug",
#                   option_list = optlist)
p <- OptionParser(usage = "%prog -a alignOutput -b assembleReports -c assembleNames -d assembleOutput -e decontamQCFiles -f decontamQCNames
                  -g decontamOutput -i decontamCloneFiles -j decontamCloneNames -k normCloneFiles -l normCloneNames -m normOutput -n debug",
                  option_list = optlist)
args <- parse_args(p)
opt <- args$options
...
alignReports_v <- args$alignReports
alignNames_v <- args$alignNames
alignOut_v <- args$alignOutput

assembleReports_v <- args$assembleReports
assembleNames_v <- args$assembleNames

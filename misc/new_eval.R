...
align_dir_v <- "~/Desktop/OHSU/tcr_spike/data/LIB170111LC/export_align/"
metadata_v <- arguments[3]
metadata_v <- "~/Desktop/OHSU/tcr_spike/data/LIB170111LC/temp"
fastq_info_v <- arguments[4]
fastq_info_v <- "~/Desktop/OHSU/tcr_spike/data/LIB170111LC/QC/remove.spikes.QC.result.txt"
qc_out_dir_v <- arguments[5]
qc_out_dir_v <- "~/Desktop/OHSU/tcr_spike/data/LIB170111LC/qc_out/"
out_dir_v <- arguments[6]
out_dir_v <- "~/Desktop/OHSU/tcr_spike/data/LIB170111LC/eval/"
...
for (i in 1:length(treatments_v)){
    ...
    for (j in 1:curr_meta_dt[,.N]){
        ...
        ## QC Output
        curr_row_v <- c(curr_sample_v, treatments_v[i], curr_num_clones_v, curr_mixcr_reads_v, curr_total_reads_v, curr_mixcr_pct_v,

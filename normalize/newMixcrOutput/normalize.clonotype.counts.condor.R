...
changeCols_v <- c("cloneCount", "cloneFraction", "Normalized clone count", "Normalized clone fraction", "nb.clone.count", "nb.clone.fraction")
...
for(index in 1:nrow(spiked_reads)) {
    ...
    if(length(indices.to.modify) > 0)   {
        ...
        count_data[indices.to.modify, `Normalized clone count` := current.spike.info$naive * count_data[indices.to.modify,`cloneCount`]]
        ...
        count_data[indices.to.modify, `nb.clone.count` := current.spike.info$nb * count_data[indices.to.modify, `cloneCount`]]
    }   #   fi
}   #   for index

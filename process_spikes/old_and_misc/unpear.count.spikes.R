...
if(direction == "REV")  {
    ...
    spikes <- sapply(spikes, function(x) as.character(reverseComplement(DNAString(x))), USE.NAMES = F)
...
for(i in 1:length(spikes))  {
    ...
    ## Count second spike occurrences
    if (spike.length == 9){
    ...
    ## If there are some reads, get their IDs from the fastq object

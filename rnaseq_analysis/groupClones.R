...
optlist <- list(
  ...
  make_option(
      c("-c", "--columns"),
      type = "character",
      default = c("cloneCount,cloneFraction,clonalSequence,aaSeqCDR3,V segments,J segments"),
  ...
  make_option(
      ...
      type = "character",
      default = '1,2,3',
      help = "Indices of fields of file name that should be KEPT. Only works when separator is '_'"),
  make_option(
    c("-l", "--log"),
    type = "logical",
...
fields_v <- args$fields; fields_v <- as.numeric(unlist(strsplit(fields_v, split = ",")))
...
### Get names based on fields
inputNames_v <- sapply(inputFiles_v, function(x) paste(strsplit(x, split = "_")[[1]][fields_v], collapse = "_"), USE.NAMES=F)
inputNames_v <- gsub(".txt", "", inputNames_v)
...
baseFile_v <- strsplit(inputNames_v[1], split = "S[0-9]+")[[1]]
toKeep_v <- paste0(baseFile_v[1], "S", gsub("S", "", unlist(meta_dt[,get(sampleCol_v)])), baseFile_v[2])
inputFiles_v <- inputFiles_v[names(inputFiles_v) %in% toKeep_v]
inputNames_v <- names(inputFiles_v)

### Get batch name
batchName_v <- strsplit(inputFiles_v[1], split = "_")[[1]][1]

### Get frequency column
column_v <- grep("[Ff]raction", columns_v, value = T)
...
clones_lsdt <- sapply(inputFiles_v, function(x) {
    ...
    y$Treatment <- meta_dt[get(sampleCol_v) %in% c(sample_v, sampNum_v), get(treatCol_v)]

...
#SBATCH --output             exportAlign_%A_%a.out        # Standard output
#SBATCH --error              exportAlign_%A_%a.err        # Standard error
...
TODO=$data/tools/todo/exportAlign.txt
...
OUTPUT=$OUT/$BASE\S$SNUM\_alignment_exported.txt
...
cmd="/usr/bin/java -Xmx15g -jar $MYBIN exportAlignments -f --preset-file $PRESET $INDEXFILE $ALIGNMENT $OUTPUT"

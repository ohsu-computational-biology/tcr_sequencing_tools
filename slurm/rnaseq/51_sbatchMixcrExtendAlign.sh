...
#SBATCH --output             extendAlign_%A_%a.out        # Standard output
#SBATCH --error              extendAlign_%A_%a.err        # Standard error
#SBATCH --array              1-1                    # sets number of jobs in array


### SET I/O VARIABLES

IN=$data/mixcr/partial2             # Directory containing all input files. Should be one job per file
OUT=$data/mixcr/extended           # Directory where output files should be written
...
TODO=$data/tools/todo/extend.txt
...
BASE="${CURRFILE%%S[0-9]*}"
SNUM="${CURRFILE%%_partial2.vdjca}"; SNUM="${SNUM##*S}"
...
INPUT=$IN/$CURRFILE
EXTENDED=$OUT/$BASE\S$SNUM\_extended.vdjca
echo "File input: "  $INPUT
echo "File output: " $EXTENDED
printf "\n\n"

### Execute

cmd="/usr/bin/java -Xmx15g -jar $MYBIN extendAlignments -f $INPUT $EXTENDED"
